...
def plot(values, x_label, x, log=False):
    ...
    y = np.array([np.average(values[x_val]) for x_val in x]) + 0.817183017731
    # adding time taken to run the following lines
    """
...
num_outliers = np.array(range(10, 91, 10))

# edges_values = pickle.load(open('results/scalability_edges.pkl', 'rb'))
outlier_values = pickle.load(open('results/scalability_outliers_best.pkl',
                                  'rb'))
